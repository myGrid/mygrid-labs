...
import uk.org.taverna.t3.workbench.canvas.diagram.part.WorkflowDiagramUpdater;
import uk.org.taverna.t3.workbench.canvas.diagram.part.WorkflowLinkDescriptor;
import uk.org.taverna.t3.workbench.canvas.diagram.part.WorkflowNodeDescriptor;
...
public class WorkflowCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	private boolean isMyDiagramElement(View view) {
		...
		return visualID == ComponentInstanceEditPart.VISUAL_ID
	...
	protected void refreshSemantic() {
		...
		for (Iterator<WorkflowNodeDescriptor> descriptorsIterator = childDescriptors
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<WorkflowLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = WorkflowVisualIDRegistry
			...
			for (Iterator<WorkflowLinkDescriptor> linkDescriptorsIterator = linkDescriptors
				...
				WorkflowLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<WorkflowLinkDescriptor> collectAllLinks(View view,
		...
		case WorkflowEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(WorkflowDiagramUpdater
						.getWorkflow_1000ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
		...
		case WorkflowOutputEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(WorkflowDiagramUpdater
						.getWorkflowInput_2002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case ComponentInstanceInputEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(WorkflowDiagramUpdater
						.getComponentInstanceInput_3001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case ComponentInstanceOutputEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(WorkflowDiagramUpdater
						.getComponentInstanceOutput_3002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection<IAdaptable> createConnections(
			Collection<WorkflowLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		...
		for (WorkflowLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					WorkflowVisualIDRegistry.getType(nextLinkDescriptor
