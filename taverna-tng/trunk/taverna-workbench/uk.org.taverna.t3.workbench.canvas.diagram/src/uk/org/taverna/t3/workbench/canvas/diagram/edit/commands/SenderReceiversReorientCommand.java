...
public class SenderReceiversReorientCommand extends EditElementCommand {
	...
	protected boolean canReorientSource() {
		...
		return CanvasBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistSenderReceivers_4003(getNewSource(), getOldTarget());
	}
	...
	protected boolean canReorientTarget() {
		...
		return CanvasBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistSenderReceivers_4003(getOldSource(), getNewTarget());
	}
	...
	protected CommandResult reorientSource() throws ExecutionException {
		getOldSource().getReceivers().remove(getOldTarget());
		getNewSource().getReceivers().add(getOldTarget());
	...
	protected CommandResult reorientTarget() throws ExecutionException {
		getOldSource().getReceivers().remove(getOldTarget());
		getOldSource().getReceivers().add(getNewTarget());
		return CommandResult.newOKCommandResult(referenceOwner);
	}
