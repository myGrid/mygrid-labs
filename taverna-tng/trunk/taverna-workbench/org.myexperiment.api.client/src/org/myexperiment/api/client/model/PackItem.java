...
public class PackItem extends Resource {
	...
	public PackItem() {
		super();
		this.setItemType(Resource.UNKNOWN); // set to unknown originally; will
											// be changed as soon as the type is
											// known
	}
	...
	public static PackItem buildFromXML(Document doc, Logger logger) {
		...
		try {
			...
			p.setUserWhoAddedTheItem(Util
					.instantiatePrimitiveUserFromElement(ownerElement));
			...
			if (root.getName().equals("internal-pack-item")) {
				...
				Element itemElement = (Element) root.getChild("item")
						.getChildren().get(0);
				if (itemElement != null) {
					p.setItem(Util
							.instantiatePrimitiveResourceFromElement(itemElement));
				}
