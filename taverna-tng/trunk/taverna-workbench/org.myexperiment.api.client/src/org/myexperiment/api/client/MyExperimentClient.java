 ...
 ******************************************************************************/
package org.myexperiment.api.client;
...
import org.myexperiment.api.client.model.Base64;
import org.myexperiment.api.client.model.Comment;
import org.myexperiment.api.client.model.File;
import org.myexperiment.api.client.model.Group;
import org.myexperiment.api.client.model.Pack;
...
import org.myexperiment.api.client.model.ServerResponse;
import org.myexperiment.api.client.model.Tag;
import org.myexperiment.api.client.model.TagCloud;
import org.myexperiment.api.client.model.User;
import org.myexperiment.api.client.model.Workflow;
import org.xml.sax.InputSource;

/**
 * Code to interact with the myExperiment API.
 * 
 * This, and most of the code in this bundle, is a heavily stripped down and
 * modified version of the code developed for the myExperiment Perspective
 * plugin for Taverna 2.x
 * (http://code.google.com/p/taverna/source/browse/taverna
 * /ui/net.sf.taverna.t2.ui-exts/trunk/perspective-myexperiment/)
 * 
...
public class MyExperimentClient {
	...
	public static final String PLUGIN_USER_AGENT = "TavernaTNG-myExperiment-client/"
			+ BUNDLE_VERSION + " Java/" + System.getProperty("java.version");
	...
	public MyExperimentClient(String baseUrl) {
		this.logger = Logger.getLogger(MyExperimentClient.class);

		// TODO: Set up settings for this client
		this.iniSettings = new Properties();
		this.iniSettings.put(INI_BASE_URL, baseUrl);
		this.iniSettings.put(INI_LOGIN, "");
		this.iniSettings.put(INI_PASSWORD, "");
		...
		this.iniSettings.put(INI_BASE_URL, BASE_URL);
	}
	...
	// Simulates a "login" action by verifying that login and password are
	// correct
	...
	public boolean doLogin(String strLogin, String strPassword)
			throws Exception {
		// Base64 encode login and password - then store these in an instance
		// variable
		AUTH_STRING = Base64.encodeBytes((strLogin + ":" + strPassword)
				.getBytes());
		...
		if (conn.getResponseCode() == HttpURLConnection.HTTP_OK) {
			...
			reader.close();
			String currentUserURI = doc.getRootElement().getAttributeValue(
					"uri");
			...
			throw new IOException("Received unexpected HTTP response code ("
					...
					+ ") while testing login credentials at " + BASE_URL
					+ "/whoami.xml");
		}
	}
	...
	public void doLoginFromStoredCredentials() {
		...
		Object oPassword = this.iniSettings
		...
		if (doc == null) {
			...
			// stored credentials will be kept to allow the user to verify and
			// edit
			// them
			// (login screen will be displayed as usual + an error message box
			// will
			// appear)
			...
			String strCurrentUserURI = doc.getRootElement().getAttributeValue(
					"uri");
			try {
				...
				this.logger
						.error("Couldn't fetch user data from myExperiment ("
								+ strCurrentUserURI
	...
	private ServerResponse doMyExperimentReceiveServerResponse(
		...
		case HttpURLConnection.HTTP_BAD_REQUEST:
			...
			// Java throws IOException if getInputStream() is used when non
			// HTTP_OK
			...
			Document errorDoc = new SAXBuilder().build(new InputSource(
					errorReader));
			errorReader.close();
	 ...
	 *            class.
	 */
	public Document getResource(int iResourceType, String strURI,
			int iRequestType) throws Exception {
		if (iRequestType == Resource.REQUEST_ALL_DATA) {
			...
			case Resource.FILE:
				strURI += "&elements="
						+ File.getRequiredAPIElements(iRequestType);
				break;
			case Resource.PACK:
				strURI += "&elements="
						+ Pack.getRequiredAPIElements(iRequestType);
				break;
			case Resource.PACK_INTERNAL_ITEM:
				strURI += "&all_elements=yes"; // TODO determine which are
												// required
				// elements
				break;
			...
			case Resource.USER:
				strURI += "&elements="
						+ User.getRequiredAPIElements(iRequestType);
				break;
			case Resource.GROUP:
				strURI += "&elements="
						+ Group.getRequiredAPIElements(iRequestType);
				break;
			case Resource.TAG:
				// this should set no elements, because default is desired at
				// the
				// moment -
				// but even having "&elements=" with and empty string at the end
				// will
				// still
				// retrieve default fields from the API
				strURI += "&elements="
						+ Tag.getRequiredAPIElements(iRequestType);
				break;
			case Resource.COMMENT:
				// this should set no elements, because default is desired at
				// the
				// moment - but even having "&elements=" with and empty string
				// at the
				// end will
				// still retrieve default fields from the API
				strURI += "&elements="
						+ Comment.getRequiredAPIElements(iRequestType);
				break;
			}
		}
	...
	public Workflow fetchWorkflowBinary(String strWorkflowURI) throws Exception {
		...
		String strEncoding = root.getChild("content").getAttributeValue(
				"encoding");
		String strDataFormat = root.getChild("content").getAttributeValue(
				"type");
	...
	public List<Workflow> getExampleWorkflows() {
		...
		try {
			...
			if (doc != null) {
				List<Element> allInternalItems = doc.getRootElement()
						.getChild("internal-pack-items")
						.getChildren("workflow");
				for (Element e : allInternalItems) {
					String itemUri = e.getAttributeValue("uri");
					Document itemDoc = this.doMyExperimentGET(itemUri)
							.getResponseBody();
					String workflowUri = itemDoc.getRootElement()
							.getChild("item").getChild("workflow")
							.getAttributeValue("uri");
					Document docCurWorkflow = this.getResource(
							Resource.WORKFLOW, workflowUri,
							Resource.REQUEST_FULL_LISTING);
	...
	public TagCloud getUserTagCloud(User user, int size) {
		...
		try {
			...
			synchronized (user.getTags()) {
				...
				Iterator<Element> iNewUserTags = doc.getRootElement()
						.getChild("tags-applied").getChildren().iterator();
				Util.getResourceCollectionFromXMLIterator(iNewUserTags,
						user.getTags());
			}

			// fetch additional required data about the tags
			Iterator<HashMap<String, String>> iTagsResourcesHashMaps = user
					.getTags().iterator();
			while (iTagsResourcesHashMaps.hasNext()) {
				...
				// fetch tag data from myExperiment (namely, number of times
				// that this
				// tag was applied)
				Document doc = this.doMyExperimentGET(strCurTagURI)
						.getResponseBody();
				Element root = doc.getRootElement();
	 ...
	 * 
	 * @param strTag
	 *            The tag to search for. This will be URL encoded before
		...
		try {
			...
			doc = this.doMyExperimentGET(
					BASE_URL + "/tagged.xml?tag=" + strUrlEncodedTag
							+ Util.composeAPIQueryElements(null))
					.getResponseBody();
		} catch (Exception e) {
	...
	public void convertTagListIntoTagCloudData(List<Tag> tags) {
		try {
			...
			for (Tag t : tags) {
				...
				t.setCount(Integer.parseInt(rootElement.getChild("count")
						.getText()));
			}
		} catch (Exception e) {
	...
	public void updateUserFavourites(User user) {
		...
		try {
			...
			List<Resource> newUserFavouritesList = Util
					.retrieveUserFavourites(doc.getRootElement());
	...
	public Document searchByQuery(QuerySearchInstance searchQuery) {
		...
		try {
			// this will URL encode the query so that it can be directly
			// inserted into
			// the search URL
			String strUrlEncodedQuery = MyExperimentClient
					.urlEncodeQuery(searchQuery.getSearchQuery());
	...
	public Map<Integer, ArrayList<Resource>> getSearchResults(QuerySearchInstance searchQuery) {
		final Map<Integer, ArrayList<Resource>> results = new HashMap<Integer, ArrayList<Resource>>();
		
		Document searchDoc = searchByQuery(searchQuery);
		
		if (searchDoc != null) {
			for (Object obj : searchDoc.getRootElement().getChildren()) {
				Resource res = Resource.buildFromXML((Element) obj, this, logger);
	
			  // check if this is a new type of resource - if so, create new ArrayList for these
			  if (!results.containsKey(res.getItemType())) {
				results.put(res.getItemType(), new ArrayList<Resource>());
			  }
	
			  // add the resource into the result set
			  results.get(res.getItemType()).add(res);
			}
		}
	...
	private String prepareWorkflowPostContent(String workflowContent,
		...
		if (workflowContent.length() > 0) {
			...
			encodedWorkflow += "<content-type>" + contentType
					+ "</content-type>"
					+ "<content encoding=\"base64\" type=\"binary\">"
					+ Base64.encodeBytes(workflowContent.getBytes())
					+ "</content>";
	...
	private void afterMyExperimentPost(ServerResponse response) {
		...
		// Workflow.buildFromXML(response.getResponseBody(),
		// logger);
		//
	...
	public ServerResponse postWorkflow(String workflowContent, String title,
		...
		try {
			String strWorkflowData = prepareWorkflowPostContent(
					workflowContent, title, description, license, sharing);
	...
	public ServerResponse updateWorkflowVersionOrMetadata(Resource resource,
			String workflowContent, String title, String description,
			String license, String sharing) {
		try {
			String strWorkflowData = prepareWorkflowPostContent(
					workflowContent, title, description, license, sharing);

			// if strWorkflowFileContent is empty; include version info for PUT
			// (since
			// workflow is being updated)
			// a POST would require data, hence strWorkflowFileContent would not
			// be
			// empty
	...
	public ServerResponse deleteFavourite(Resource resource) {
		try {
			...
			String strFavouriteURI = root.getChild("favourite")
					.getAttributeValue("uri");
