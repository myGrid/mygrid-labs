...
public class Resource implements Comparable<Resource>, Serializable {
	...
	// particular purpose)
	public static final int REQUEST_ALL_DATA = 5000; // essentially obtains all
														// data that API
														// provides
	public static final int REQUEST_FULL_PREVIEW = 5005; // used to get all data
															// for preview in a
															// browser window
	public static final int REQUEST_FULL_LISTING = 5010; // used for displaying
															// results of
															// searches by query
															// / by tag
	public static final int REQUEST_SHORT_LISTING = 5015; // used for displaying
															// items in 'My
															// Stuff' tab
	...
	public static final int REQUEST_DEFAULT_FROM_API = 5100; // used when
																// default
																// fields that
																// come from the
																// API are
	...
	public boolean equals(Object other) {
		...
		return (this.itemType == otherRes.itemType
				&& this.uri.equals(otherRes.uri) && this.resource
				.equals(otherRes.resource));
	}
	...
	public static int getResourceTypeFromVisibleName(String name) {
		if (name.toLowerCase().equals(
				Resource.WORKFLOW_VISIBLE_NAME.toLowerCase()))
			return (Resource.WORKFLOW);
		else if (name.toLowerCase().equals(
				Resource.FILE_VISIBLE_NAME.toLowerCase()))
			return (Resource.FILE);
		else if (name.toLowerCase().equals(
				Resource.PACK_VISIBLE_NAME.toLowerCase()))
			return (Resource.PACK);
		else if (name.toLowerCase().equals(
				Resource.USER_VISIBLE_NAME.toLowerCase()))
			return (Resource.USER);
		else if (name.toLowerCase().equals(
				Resource.GROUP_VISIBLE_NAME.toLowerCase()))
			return (Resource.GROUP);
		else if (name.toLowerCase().equals(
				Resource.TAG_VISIBLE_NAME.toLowerCase()))
			return (Resource.TAG);
		else if (name.toLowerCase().equals(
				Resource.COMMENT_VISIBLE_NAME.toLowerCase()))
	...
	public static Resource buildFromXML(Element docRootElement,
			MyExperimentClient client, Logger logger) {
		Resource res = null;
		switch (Resource.getResourceTypeFromVisibleName(docRootElement
				.getName())) {
		case Resource.WORKFLOW:
