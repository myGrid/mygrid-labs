...
import org.myexperiment.api.client.Util;
...
public class Workflow extends Resource {
	...
	public boolean isTavernaWorkflow() {
		return (contentType.equals(Workflow.MIME_TYPE_TAVERNA_1) || contentType
				.equals(Workflow.MIME_TYPE_TAVERNA_2));
	}
	...
	public static Workflow buildFromXML(Element docRootElement, Logger logger) {
		...
		try {
			// Access type
			w.setAccessType(Util.getAccessTypeFromXMLElement(docRootElement
					.getChild("privileges")));
			...
			w.setUploader(Util
					.instantiatePrimitiveUserFromElement(uploaderElement));
			...
			// License
			w.setLicense(License.getInstance(docRootElement
					.getChildText("license-type")));
			...
			w.getAttributions().addAll(
					Util.retrieveAttributions(docRootElement));
			...
			if (componentsElement != null) {
				...
				if (linksElement != null) {
					...
					for (Element e : linksNodes) {
						...
						String strSource = e.getChild("source").getChildText(
								"node")
								+ (e.getChild("source").getChildText("port") == null ? ""
										: (":" + e.getChild("source")
												.getChildText("port")));
						curLink.put("source", strSource);
						String strSink = e.getChild("sink")
								.getChildText("node")
								+ (e.getChild("sink").getChildText("port") == null ? ""
										: (":" + e.getChild("sink")
												.getChildText("port")));
